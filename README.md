...
Firstly, there is no open source data of all the food establishments so I am forced to use a combination of foursquare + google places + OSM. It would be great if I had access to delivery service apis such as grab, foodpanda but they fall under paid services. Also it would be really cool if I am able to have access to the database of NEA, gov.sg showcasing all the hawker stalls. Currently the only publicly available database of it shows the hawker centres only which is not ideal in finding the individual stalls IN the hawker centres. The link can be found here : https://data.gov.sg/datasets/d_4a086da0a5553be1d89383cd90d07ecd/view. Foursquare is inaccurate as it will sometimes lists food establishments that do not exists or have already closed down. Not to mention, since we are trying to match the food establishments to the user input, foursquare occasionally recommends food that do not match the user input. Google places is accurate however is limited to 20 results per page and since I am trying to make the results cumulative based off radius the results for google places will be capped at 20 only. OSM suffers the same as foursquare both are inaccurate and have little data about Singapore. Not only that, another challenge I faced is that most hawker stalls or "hidden gems" are not reliably tagged which results in the app not being able to find them.
